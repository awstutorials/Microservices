...
import com.in28minutes.microservices.limitsservice.auth.model.User;
...
public class CognitoTokenVerifier {
     ...
     *
     * */
    public User checkRequestAgainstUserPool(HttpServletRequest request) {
    ...
    public  User checkToken(String idToken, HttpServletRequest request)  {

        // validate JWT token
        AwsCognitoJwtParserUtil.validateJWT(idToken);

        JWTClaimsSet jwtClaimsSet = validateAWSJwtToken(idToken);
